...
class HDFCParser(BaseParser):
    ...
    def parse(self, email: dict) -> TransactionDraft:
        ...
        ref_match = re.search(r"Reference:\s*([A-Z0-9]+)|Ref\.?\s*No\.?:\s*([A-Z0-9]+)|Ref\s*No\.?:\s*([A-Z0-9]+)|Ref:\s*([A-Z0-9]+)", body, re.IGNORECASE)
        if ref_match:
            # Get the first non-None group
            draft.reference_number = ref_match.group(1) or ref_match.group(2) or ref_match.group(3) or ref_match.group(4)
