...
class AxisParser(BaseParser):
    ...
    def can_parse(self, email: dict) -> bool:
        """Check if email is from Axis Bank."""
        sender = email.get("sender", "").lower()
        
        axis_senders = ["alerts@axisbank.com", "noreply@axisbank.com"]
        
        return any(pattern in sender for pattern in axis_senders)
    
    def get_sender_patterns(self) -> list[str]:
        return ["alerts@axisbank.com", "noreply@axisbank.com"]
