"""Kotak Mahindra Bank email parser."""
...
class KotakParser(BaseParser):
    """Parser for Kotak Mahindra Bank transaction emails."""
    
    def can_parse(self, email: dict) -> bool:
        """Check if email is from Kotak Bank."""
        sender = email.get("sender", "").lower()
        
        kotak_senders = ["alerts@kotak.com", "noreply@kotak.com", "alerts@kotakbank.com"]
        
        return any(pattern in sender for pattern in kotak_senders)
    
    def get_sender_patterns(self) -> list[str]:
        return ["alerts@kotak.com", "noreply@kotak.com", "alerts@kotakbank.com"]
