...
class AmexParser(BaseParser):
    ...
    def parse(self, email: dict) -> TransactionDraft:
        ...
        amount_match = re.search(
            r"(?:USD|INR|Rs\.?)\s*[\d,]+\.?\d*", body, re.IGNORECASE
        )
        if amount_match:
            amount_str = (
                amount_match.group()
        ...
        if (
            "debit" in subject.lower()
            or "spent" in body.lower()
            or "purchase" in body.lower()
        ):
            draft.transaction_type = "debit"
        elif (
            "credit" in subject.lower()
            or "received" in body.lower()
            or "refund" in body.lower()
        ):
