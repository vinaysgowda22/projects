...
class FailedEmailRepository(BaseRepository[FailedEmail]):
    ...
    def get_by_gmail_id(self, gmail_id: str) -> Optional[FailedEmail]:
        ...
        return (
            self.session.query(FailedEmail).filter(FailedEmail.gmail_id == gmail_id).first()
        )
    ...
    def increment_retry(self, failed_email: FailedEmail) -> FailedEmail:
        ...
        return self.update(
            failed_email, retry_count=failed_email.retry_count + 1, last_retry_at=datetime.now()
        )
