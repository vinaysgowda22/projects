...
from backend.repositories.transaction_repository import TransactionRepository, get_transaction_repository
...
class DuplicateDetector:
    ...
    def is_duplicate(
        ...
        if reference_number:
            existing = self._find_by_reference_number(reference_number, exclude_transaction_id)
            if existing:
    ...
    def find_duplicates_for_transaction(
        ...
        if transaction.gmail_id:
            gmail_matches = session.query(Transaction).filter(
        ...
        if transaction.reference_number:
            ref_matches = session.query(Transaction).filter(
        ...
        fuzzy_matches = session.query(Transaction).filter(
