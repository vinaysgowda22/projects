    ...
    def _get_credentials_from_keychain(self) -> Optional[Credentials]:
        ...
        try:
            token_json = keyring.get_password(
                self.KEYCHAIN_SERVICE, self.KEYCHAIN_USERNAME
            )
            if token_json:
    ...
    def _save_credentials_to_keychain(self, creds: Credentials) -> None:
        ...
        try:
            ...
            keyring.set_password(
                self.KEYCHAIN_SERVICE, self.KEYCHAIN_USERNAME, token_json
            )
        ...
        while len(emails) < max_results:
            ...
            try:
                ...
                if e.resp.status == 404:
                    logger.warning(
                        f"History ID {current_history_id} not found, may be too old"
                    )
                    break
                raise
